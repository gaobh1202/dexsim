...
import dexmimicgen
from dexmimicgen.models.objects import HammerObject
...
_SHELF_TOP_Z       = 0.1589   # m — shelf 顶面距 shelf body 原点高度（scale=1500 实测）
_NAIL_SLIDE_RANGE        = "-0.005 0.13"
_NAIL_SLIDE_FRICTIONLOSS = "10"
_NAIL_SLIDE_DAMPING      = "0.3"
# 锤击速度判定阈值：漂移 ≈ 0.006 m/s，锤击应远大于此
_NAIL_DRIVE_VEL_THRESHOLD = 0.02   # m/s
# 成功判定：钉入 >= 80 mm
_SUCCESS_DEPTH = 0.08    # m

DEFAULT_HAMMER_SCALE   = 0.28


class HammerShelfNail(ManipulationEnv):
    """
    锤击打钉场景：
      - WoodShelf（静态，scale=1500）放在桌面
      - nail_red_star 作为 shelf 的子 body，tip 位于货架顶面几何中心，
        通过 slide joint（axis="0 0 -1"）等待锤击；重置时 qpos=0 初始化
      - HammerObject（自由关节）供机械臂抓取后将钉子打入货架

    nail 在 XML 合并后经 _reparent_nail_under_shelf 挂入 wood_shelf_main：
      freejoint → slide joint (axis 0 0 -1)  +  nail ↔ shelf 接触排除
    """
    ...
    def __init__(
        ...
        hammer_scale=DEFAULT_HAMMER_SCALE,
    ):
    ...
    def _load_model(self):
        ...
        self.model = ManipulationTask(
            ...
            mujoco_objects=[self.shelf, self.nail_red, self.hammer],
        )
    ...
    def _get_placement_initializer(self):
        # nail_red 由 slide joint 控制，shelf 由 _place_shelf 控制，只放置 hammer
    ...
    def _reparent_nail_under_shelf(self, xml_str):
        """
        nail_red_main: worldbody → wood_shelf_main 子节点
          - freejoint 替换为 slide joint（axis 0 0 -1）
          - pos 设为货架顶面几何中心（0, 0, _SHELF_TOP_Z）
          - 添加 nail ↔ shelf 接触排除
        """
        root      = ET.fromstring(xml_str)
        ...
        # nail tip 位于货架顶面几何中心
        nail_body.set("pos",  f"0 0 {_SHELF_TOP_Z:.4f}")
        ...
        slide_j.set("frictionloss", _NAIL_SLIDE_FRICTIONLOSS)
    ...
    def step(self, action):
        ...
        v = abs(float(self.sim.data.qvel[self._nail_dof_adr]))
        q = float(self.sim.data.qpos[self._nail_qpos_adr])
        if v > _NAIL_DRIVE_VEL_THRESHOLD:
            # 锤击中：更新锁定深度（只进不退）
            self._nail_frozen_qpos = max(self._nail_frozen_qpos, q)
        else:
            # 缓慢漂移：冻结到上次锤击深度
            self.sim.data.qpos[self._nail_qpos_adr] = self._nail_frozen_qpos
            self.sim.data.qvel[self._nail_dof_adr]  = 0.0
