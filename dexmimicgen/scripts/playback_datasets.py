...
def normalize_video_frame(frame):
    """
    Normalize renderer output to contiguous uint8 HxWx3 for video backends.
    """
    if isinstance(frame, tuple):
        # Some renderers return (rgb, depth) or similar tuples.
        frame = frame[0]
...
class OpenCVVideoWriter:
    """
    Minimal imageio-like writer API backed by cv2.VideoWriter.
    """
        ...
        if write_video:
            if video_count % video_skip == 0:
                ...
                for cam_name in camera_names:
                    render_out = env.sim.render(
                        height=512, width=512, camera_name=cam_name
                    )
                    im = normalize_video_frame(render_out)
                    # Flip vertical axis to match previous behavior / viewer convention.
                    im = im[::-1, :, :]
                    video_img.append(im)
...
def playback_dataset(args):
    ...
    video_writer = None
    if write_video:
        video_writer = make_video_writer(args.video_path, fps=20)
